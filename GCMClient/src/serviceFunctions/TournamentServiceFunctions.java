...
public class TournamentServiceFunctions {
	...
	public static List<Tournament> getTournaments() {
		...
		List<Tournament> tournaments = ClientBuilder.newClient().target(serverURI).path("/tournamentlist")
				.request(MediaType.APPLICATION_XML).get(new GenericType<List<Tournament>>() {
				});
	...
	public static Tournament getTournament(int id) {
		...
		Tournament tournament = ClientBuilder.newClient().target(serverURI).path("/tournament/" + id)
				.request(MediaType.APPLICATION_XML).get(new GenericType<Tournament>() {
				});
